...
//更新节点的max数值
void UpdateMaxValue(IBnode* node)
{
	while (node != nil)
	{
		node->max = Max(node->int_high, node->left->max, node->right->max);
		node = node->parent;
	}
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void LeftRotate(IBnode** root, IBnode* nodex)
{
	if (nodex->right != nil)
	{
		...
		//更新旋转后的两个节点的Max值
		UpdateMaxValue(nodex);
		UpdateMaxValue(nodey);
	}
	else
	{
...
{
	IBnode* nodey = nil;		//y用于记录：当前扫描节点的父节点
	IBnode* nodex = *root;		//从root开始扫描

	while (nodex != nil)		//查找插入位置
	{
		nodey = nodex;
		if (nodez->int_low < nodex->int_low)
			nodex = nodex->left;
		else
			nodex = nodex->right;
	}
	...
	{
		if (nodez->int_low < nodey->int_low)	//左子插入
			nodey->left = nodez;
		else							//右子插入
			nodey->right = nodez;
	}
	...
	//插入后更新nodez的max值
	UpdateMaxValue(nodez);

	IBInsertFixup(root, nodez);
}
...
void IBInsertFixup(IBnode** root, IBnode* nodez)
{
	while (1)
	{
		...
		else
		{
			...
			{
				IBnode* nodey = ((nodez->parent)->parent)->right;		//y是z的叔叔
			...
			{	//if (nodez->parent = ((nodez->parent)->parent)->right)
				IBnode* nodey = ((nodez->parent)->parent)->left;		//y是z的叔叔
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
IBnode* TreeSuccessor(IBnode* root)
{
	IBnode* leftfirst = root;
...
{
	IBnode *nodex = nil, *nodey = nil;			//x是连接点，y是真正物理删除节点，z是指向删除
	...
	if (nodey != nodez)
		nodez->int_low = nodey->int_low;
	if (nodey->color == BLACK)	//如果删除的节点是红色节点，则无需调整
		IBDeleteFixup(root, nodex);
	return nodey;
}
...
void  IBDeleteFixup(IBnode** root, IBnode* nodex)
{
	while (1)		//x始终作为矛盾的存在。
	{
		...
		{								//x是双黑节点。
			...
			{
				IBnode* nodew = nodex->parent->right;	//w是x的兄弟节点
			...
			{
				IBnode* nodew = nodex->parent->left;	//w是x的兄弟节点
