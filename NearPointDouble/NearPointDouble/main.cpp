...
void gen_random_set()
{
	...
	for (i = 0; i<N; i++)
	{
		x[i] = ((double)(rand() % N)) / 10;
		cout << x[i] << ' ';
	}
	cout << endl;
	for (i = 0; i < N; i++)
	{
		y[i] = ((double)(rand() % N)) / 10;
		cout << y[i] << ' ';
	}
